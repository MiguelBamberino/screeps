...
class RoomNode{
    ...
    runTowers(){
     //   return
        ...
        for(var id of hostileIds){
            ...
            if(hostileIds.length<3 && enemy.owner.username==='Invader'){
               target= enemy;break;
            }
            if(range<=15 ){
    ...
    inRecoveryMode(){
       // if(Game.time%10===0)this._recoverMode=undefined;
        ...
        if(!this.spawnFastFillerReady || this.workforce_quota.harvester.count<srcs.length || this.workforce_quota.tanker.count===0){
    ...
    decideWorkforceQuotas(){
        ...
        if(controller.level>1){
            ...
            if(this.upgradeRate!=RATE_VERY_FAST && !this.funnelRoomName && storage && !storage.haveSpaceFor(50000)){
            ...
            if(this.spawnFastFillerReady && controller.haveContainer()){
